const Card = () => {
  return (
    <div className="mx-auto container">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-6 w-full bg-white p-4">
       

        <div className="p-6 md:p-10 rounded-xl bg-blue-50">
         

          <img src="/Add.png" className="h-8 w-8" alt="" />

          <h3 className="mt-4 text-base md:text-xl font-medium text-gray-800">
          Add your server
          </h3>
          <p className="mt-4 text-base md:text-lg text-gray-600">
          Begin by adding your server to our platform. It's as simple as a few clicks. Just provide the necessary server details, and you're good to go
          </p>
        </div>

        <div className="p-6 md:p-10 rounded-xl bg-blue-50">
          <div className="inline-flex rounded-full bg-white p-4">
            <svg
              className="w-8 h-8 text-primary fill-current text-blue-600"
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 576 512"
            >
              <defs></defs>
              <path
                d="M560 224h-29.51a159.88 159.88 0 0 0-37.38-52.46L512 96h-32c-29.4 0-55.39 13.5-73 34.32-7.57-1.1-15.12-2.32-23-2.32H256c-94.82 0-160 78.88-160 160a159.75 159.75 0 0 0 64 128v80a16 16 0 0 0 16 16h64a16 16 0 0 0 16-16v-48h128v48a16 16 0 0 0 16 16h64a16 16 0 0 0 16-16v-80.72A160.37 160.37 0 0 0 511.28 384H560a16 16 0 0 0 16-16V240a16 16 0 0 0-16-16zm-128 64a16 16 0 1 1 16-16 16 16 0 0 1-16 16z"
                className="fa-secondary opacity-40"
              ></path>
              <path d="M51.26 255.52a24 24 0 0 1-18.74-28.3C34.74 215.82 45.4 208 57 208h1a6 6 0 0 0 6-6v-20a6 6 0 0 0-6-6C29.5 176 4.1 196.4.47 224.62a54.64 54.64 0 0 0-.47 7.23A56.08 56.08 0 0 0 56 288h40a155.05 155.05 0 0 1 3.37-32H56a23.63 23.63 0 0 1-4.74-.48zM432 256a16 16 0 1 0 16 16 16 16 0 0 0-16-16zM306.5 0a96 96 0 0 0-88.81 132.51A162.64 162.64 0 0 1 256 128h128a104.31 104.31 0 0 1 12.71.88A96.06 96.06 0 0 0 306.5 0z"></path>
            </svg>
          </div>

          <h3 className="mt-4 text-base md:text-xl font-medium text-gray-800">
          Connect your github
          </h3>
          <p className="mt-4 text-base md:text-lg text-gray-600">
          Link your project by connecting it to your GitHub repository. This step ensures seamless integration with your codebase.
          </p>
        </div>

        <div className="p-6 md:p-10 rounded-xl bg-blue-50">
          <div className="inline-flex rounded-full bg-white p-4">
            <svg
              className="w-8 h-8 text-primary fill-current text-blue-600"
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 576 512"
            >
              <defs></defs>
              <path
                d="M560 224h-29.51a159.88 159.88 0 0 0-37.38-52.46L512 96h-32c-29.4 0-55.39 13.5-73 34.32-7.57-1.1-15.12-2.32-23-2.32H256c-94.82 0-160 78.88-160 160a159.75 159.75 0 0 0 64 128v80a16 16 0 0 0 16 16h64a16 16 0 0 0 16-16v-48h128v48a16 16 0 0 0 16 16h64a16 16 0 0 0 16-16v-80.72A160.37 160.37 0 0 0 511.28 384H560a16 16 0 0 0 16-16V240a16 16 0 0 0-16-16zm-128 64a16 16 0 1 1 16-16 16 16 0 0 1-16 16z"
                className="fa-secondary opacity-40"
              ></path>
              <path d="M51.26 255.52a24 24 0 0 1-18.74-28.3C34.74 215.82 45.4 208 57 208h1a6 6 0 0 0 6-6v-20a6 6 0 0 0-6-6C29.5 176 4.1 196.4.47 224.62a54.64 54.64 0 0 0-.47 7.23A56.08 56.08 0 0 0 56 288h40a155.05 155.05 0 0 1 3.37-32H56a23.63 23.63 0 0 1-4.74-.48zM432 256a16 16 0 1 0 16 16 16 16 0 0 0-16-16zM306.5 0a96 96 0 0 0-88.81 132.51A162.64 162.64 0 0 1 256 128h128a104.31 104.31 0 0 1 12.71.88A96.06 96.06 0 0 0 306.5 0z"></path>
            </svg>
          </div>

          <h3 className="mt-4 text-base md:text-xl font-medium text-gray-800">
          Deploy
          </h3>
          <p className="mt-4 text-base md:text-lg text-gray-600">
          Once your server is set up, and your GitHub repository is connected, it's time to deploy your project. A single click will initiate the deployment process, making your application accessible to the world. It's that easy!
          </p>
        </div>
      </div>
    </div>
  );
};

export default Card;
